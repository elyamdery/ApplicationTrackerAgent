...
class ScreenRecorder:
    ...
    def __init__(self, output_dir="test_recordings", fps=10, codec="XVID", use_unique_folders=False):
        """
        ...
        Args:
            ...
            use_unique_folders (bool): Whether to create a unique folder for each test
        """
        self.output_dir = output_dir
        self.fps = fps
        self.codec = codec
        self.use_unique_folders = use_unique_folders
        self.recording = False
        self.thread = None
        self.out = None
        self.current_test_folder = None
    ...
    def start_recording(self, test_name=None):
        ...
        if test_name:
            ...
            # Create a unique folder for this test if requested
            if self.use_unique_folders:
                self.current_test_folder = os.path.join(self.output_dir, test_name)
                Path(self.current_test_folder).mkdir(parents=True, exist_ok=True)
                self.output_file = os.path.join(self.current_test_folder, filename)
            else:
